...
Токен бота нужен до первого `make up`: демон `cmd/bot` входит в стек и без
`TELEGRAM_BOT_TOKEN` не стартует. Как его получить — в разделе
[Бот: живые обновления канала](#бот-живые-обновления-канала).
...
Дальше бот поднимается вместе со всем стеком, отдельной команды не нужно:
...
Инструкции для модели лежат отдельными файлами в `api/prompts/` — правятся как
обычный текст, без Go-литералов. Они вшиты в бинарник, поэтому после правки
нужна пересборка: `docker compose build worker`.
