  ...
  >
    <div v-if="post.cover" class="m-1 shrink-0 overflow-hidden rounded-3xl md:hidden">
      <img
        ...
        :alt="post.title"
        loading="lazy"
        class="ui-zoom h-40 w-full object-cover"
      />
    </div>
    ...
    >
      ...
      <time :datetime="post.createdAt" class="pt-3 text-sm text-gray-500">{{ publishedAt }}</time>
    </div>
    ...
    <div v-if="post.cover" class="m-1 hidden w-[30%] shrink-0 overflow-hidden rounded-3xl md:block">
      <img
        ...
        :alt="post.title"
        loading="lazy"
        class="ui-zoom h-full w-full object-cover"
      />
    </div>
  </RouterLink>
</template>
